...
9. Canvas re-renders: each Parent row's Nested Repeater shows only the records whose reference field matches that row's ID. Each parent row displays skeleton items (no content) whose count matches the real per-row filtered result count.
...
11. The configure panel for the Nested Repeater's list context shows a **`Related {ParentContext}`** property row (e.g. `Related {Categories}`) in the **List Configuration** section. In the connected state, the row displays a bound-property chip: `Categories (source) · Current item.ID ✕`. The chip's `✕` button disconnects the relationship (see disconnected state below).

**Configure Panel — `Related {ParentContext}` Row:**

| State | Visual | Behavior |
|---|---|---|
| **Connected** | Bound chip: `[Categories (source)] [Current item.ID ✕]` | The per-row filter is active. Each parent row's Nested Repeater resolves only the records matching that row's ID. Clicking `✕` disconnects the relationship — all items are shown unfiltered (no filter count is added to the filter row); the chip is replaced by a "Connect to Categories →" link. |
| **Disconnected** | Inline link: `Connect to Categories →` (also accessible via the bind icon button) | No per-row filter is active. The Nested Repeater shows all records in the related collection, unfiltered. Clicking the link or the bind icon opens the field-picker dropdown titled "Connect · Related {Categories}", pre-scoped to the parent context. |
| **Reconnecting via field picker** | Dropdown: "Connect · Related {Categories}", field list pre-scoped to parent context fields | Builder selects the reference field (e.g. `_id`). On confirm, the relationship is restored to the connected state; canvas re-renders with skeleton items at the correct per-category count (no content is populated). |
...
| Aspect | Detail |
|---|---|
| **Impacted Areas** | Binding platform (floating binding panel, scope-badge mechanism, new `Related collections` section), Repeater inspector (`renderSettingsRepeaterOnRepeater()`), **Repeater configure panel** (`renderListSettings()` — new `Related {ParentContext}` property row with connected/disconnected states and field-picker reconnect flow), CMS data-resolution runtime (`wixData.queryReferenced()` / Blocks `include()` for M1; `wixData.query().hasSome()` for M2), Repeater context card. |
| **Required Capability Changes** | **M1:** Add `This item` scope to the binding-picker scope handling; surface array fields and multi-reference fields of the Parent item's collection at the top of the picker. **M2:** Introspect the schema of all available collections at picker-open time to identify those with a reference field pointing to the parent collection; surface them in a `Related collections` section; automatically configure the per-row filter on Apply. Both mechanisms share the same parent-row context infrastructure (FR-003). **M2 configure panel:** Add a `Related {ParentContext}` row to the List Configuration section of the nested repeater's configure panel. The row must reflect the current relationship active/inactive state and support: (a) disconnecting via chip `✕` — sets relationship inactive, shows all records unfiltered; (b) reconnecting via field picker — pre-scoped to the parent context, restores active state and per-row skeleton counts on confirm without populating actual item content. The field picker must appear above the configure panel's floating shell (z-index management required). |
| **Data & Storage Implications** | **M1 binding stores:** source collection, field path (Child array or multi-reference), scope marker = `this_item`. **M2 binding stores:** related collection name, reference field name on the child, scope marker = `related_context`; the per-row filter is computed at render time from the parent item's ID. The relationship has an explicit **active / inactive** state: active = per-row filter on; inactive = all records shown unfiltered. The builder controls this state via the `Related {ParentContext}` chip in the configure panel (connect/disconnect). The binding persists the active state and the chosen reference field. No new schema change required for either. |
